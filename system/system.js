 ...
 */
database.connect(config.mongo.dsn);
...
io.set('authorization', function(data, accept) {
	if (data.headers.cookie) {
		...
		var Session = database.model("Session").findById(data.sessionID, function(err, session){
			if (err) {
		...
		/*database.model("Session").get(data.sessionID, function(err, session) {
			if (err) {
				// if we cannot grab a session, turn down the connection
				accept(err.message, false);
			} else {
